---
title: "Alinhamento com Cell Ranger"
format:
  html:
    toc: true
    toc-depth: 3
...
1,Sample002,Sample002,SI-GA-B2
```

- A seção `[Data]` delimita o início da tabela.
- `Lane` representa o número da linha de fluxo onde a amostra foi carregada.
- `Index` refere-se ao barcode utilizado para identificar a amostra.

Para mais detalhes, consulte a [documentação da Illumina](https://support.illumina.com). deve seguir o padrão Illumina, especificando amostras, lanes e barcodes.
- Saída: arquivos FASTQ organizados por amostra.
...
- **SI-GA-A1_1**: nome da amostra, herdado do campo `Sample_Name` do Sample Sheet.
- **S1**: identificador da amostra atribuído automaticamente pelo `bcl2fastq`.
- **L001**: número da lane (neste caso, lane 1).
- **R1**: tipo da leitura:
  - `R1` = Read 1 (transcrito)
  - `R2` = Read 2 (UMI)
  - `I1` = Index Read 1 (barcode)
